...
let rec eval env ((cstack, stack, ((st, i, o, r) as c)) as conf) prg =
  ...
  | ins :: ps -> match ins with
                  ...
                  | CALL fname -> eval env ((ps, st)::cstack, stack, c) (env#labeled fname)
...
let rec compile =
  ...
  | Expr.Call (fname, es), l -> (List.concat (List.map (function x -> expr (x, l)) (List.rev es))) @ [CALL (label_of_proc fname es)]
  in
  ...
  | Stmt.Return None   , l -> [END], l
  | Stmt.Return (Some ret), l -> (expr (ret, l)) @ [END], l
